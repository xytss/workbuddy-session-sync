...
- 当前准备同步多少个对话。
- 自动同步是否开启。
- 每个对话现在属于哪个账号，账号列优先完整显示账号名称。
- 每个对话的完整会话 ID；标题、账号和会话 ID 使用连续的左对齐只读列，仅由原生表头边界区分。
...
鼠标停留在「立即同步」「重新读取」或「在 WorkBuddy 中打开」按钮上时，按钮右侧会直接显示完整说明；
使用键盘把焦点移到按钮上也会显示。说明固定在窗口内部，不会被屏幕边缘或任务栏裁切。
在会话列表中高亮一条会话后，可以按 `Esc` 取消高亮。

窗口还原时会优先压缩会话标题列，保证账号名称和完整会话 ID 不被挤出窗口；窗口最大化时，
标题列不会无限拉宽，右侧信息列会保持在一起，剩余空间留在表格末尾。为避免误操作，这些只读列
不能拖动改宽。
